...
/**
 * Order status import controller factory for CSV files
 *
 * @package Controller
 * @subpackage Jobs
 */
	...
	implements \Aimeos\Controller\Jobs\Common\Factory\Iface
{
	...
	public static function create( \Aimeos\MShop\Context\Item\Iface $context, \Aimeos\Bootstrap $aimeos, string $name = null ) : \Aimeos\Controller\Jobs\Iface
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/jobs/order/status/csv/name', 'Standard' );
		}

		$iface = '\\Aimeos\\Controller\\Jobs\\Iface';
		$classname = '\\Aimeos\\Controller\\Jobs\\Order\\Status\\Csv\\' . $name;
		 ...
		 * @see controller/jobs/order/status/csv/decorators/global
		 */
		return self::addControllerDecorators( $context, $aimeos, $controller, 'order/status/csv' );
	}
}
