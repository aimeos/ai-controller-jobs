...
/**
 * Supplier import controller factory for CSV files.
 *
 * @package Controller
 * @subpackage Jobs
 */
class Factory extends \Aimeos\Controller\Jobs\Common\Factory\Base implements
    \Aimeos\Controller\Jobs\Common\Factory\Iface
{
    /**
    ...
    public static function create(
        ...
        if ($name === null) {
            $name = $context
                ->getConfig()
                ->get('controller/jobs/supplier/import/csv/name', 'Standard');
        }

        if (ctype_alnum($name) === false) {
            $classname = is_string($name)
                ? '\\Aimeos\\Controller\\Jobs\\Supplier\\Import\\Csv\\' . $name
                : '<not a string>';
         ...
         * @see controller/jobs/supplier/import/csv/decorators/global
         */
        return self::addControllerDecorators(
            $context,
            $aimeos,
            $controller,
            'supplier/import/csv'
