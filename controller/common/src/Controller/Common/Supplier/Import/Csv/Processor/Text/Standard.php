    ...
    implements \Aimeos\Controller\Common\Supplier\Import\Csv\Processor\Iface
{
    ...
    public function __construct(
        ...
        if ($this->listTypes === null) {
            ...
            $search->setConditions(
                $search->compare('==', 'supplier.lists.type.domain', 'text')
            );
        ...
        $search = $manager->createSearch()->setSlice(0, 0x7fffffff);
        $search->setConditions(
            $search->compare('==', 'text.type.domain', 'supplier')
        );
    ...
    public function process(
        \Aimeos\MShop\Supplier\Item\Iface $supplier,
        array $data
    ) {
        $listManager = \Aimeos\MShop::create(
            $this->getContext(),
            'supplier/lists'
        );
        ...
        foreach ($listItems as $listItem) {
            if (($refItem = $listItem->getRefItem()) !== null) {
                $listMap[$refItem->getContent()][$refItem->getType()][
                    $listItem->getType()
                ] = $listItem;
            }
        }
        ...
        foreach ($map as $pos => $list) {
            ...
            $label = mb_strcut(
                $this->getValue($list, 'text.content', ''),
                0,
                255
            );
        ...
        $supplier->deleteListItems($listItems, true);
    ...
    protected function checkEntry(array $list)
    {
        ...
        if (
            ($type = $this->getValue($list, 'supplier.lists.type')) &&
            !isset($this->listTypes[$type])
        ) {
            $msg = sprintf(
        ...
        if (
            ($type = $this->getValue($list, 'text.type')) &&
            !isset($this->types[$type])
        ) {
